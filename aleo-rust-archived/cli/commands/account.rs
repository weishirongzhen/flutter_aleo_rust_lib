...
use snarkvm::prelude::{Address, Ciphertext, Environment, FromBytes, PrivateKey, ViewKey, TestRng};
...
impl AccountNew {
    ///把16进制字符串转为 [u8] 类型
    pub fn hex_to_u8_bytes(hex_str: &str) -> Vec<u8> {
        let bytes = hex::decode(hex_str).unwrap();
        return bytes;
    }
    pub fn private_key_new() -> String {
        let pk = PrivateKey::<CurrentNetwork>::new(&mut StdRng::from_entropy()).unwrap();
        return pk.to_string();
    }
    /// 通过32位 16进制字符转换成的[u8]input 生成私钥， 比如长度64的16进制：f4d5f3fcc76853544c434423fc06de9daec8e8f5123127b5ee2743c68a21e41d， （2个字符一个字节）
    pub fn private_key_from_seed(seed: &[u8]) -> PrivateKey<CurrentNetwork> {
        let seed_real: [u8; 32] = seed.try_into().unwrap();
        let field = <CurrentNetwork as Environment>::Field::from_bytes_le_mod_order(&seed_real);
        let pk = PrivateKey::<CurrentNetwork>::try_from(FromBytes::read_le(&*field.to_bytes_le().unwrap()).unwrap()).unwrap();
        return pk;
    }
    pub fn private_key_to_string(private_key: PrivateKey<CurrentNetwork>) -> String {
        return private_key.to_string();
    }

    pub fn private_key_from_string(str: &str) -> PrivateKey<CurrentNetwork> {
        return PrivateKey::<CurrentNetwork>::from_str(str).unwrap();
    }

    pub fn private_key_to_view_key(private_key: PrivateKey<CurrentNetwork>) -> ViewKey<CurrentNetwork> {
        let view_key = ViewKey::try_from(&private_key).unwrap();
        return view_key;
    }
    pub fn view_key_to_string(view_key: ViewKey<CurrentNetwork>) -> String {
        return view_key.to_string();
    }

    pub fn view_key_to_address(view_key: ViewKey<CurrentNetwork>) -> Address<CurrentNetwork> {
        let address = Address::try_from(&view_key).unwrap();
        return address;
    }

    pub fn address_to_string(address: Address<CurrentNetwork>) -> String {
...
mod tests {
    ...
    use snarkvm::prelude::{Environment, FromBytes, TestRng};
    use aleo_rust::ToBytes;
    ...
    fn new_seed() {
        let pk = PrivateKey::<CurrentNetwork>::new(&mut StdRng::from_entropy()).unwrap();
        println!("random private key = {}", pk.to_string());
        let seed = "f4d5f3fcc76853544c434423fc06de9daec8e8f5123127b5ee2743c68a21e41d".to_string();
        // this seed's private key should be APrivateKey1zkpJG4gRXtaThk3gzkWksaunfEaCSFSF8xaqRbiNfcakHU6
        let bytes = hex::decode(seed).unwrap();
        let bytes_slice: &[u8] = &bytes;
        let seed_real: [u8; 32] = bytes_slice.try_into().unwrap();
        let field = <CurrentNetwork as Environment>::Field::from_bytes_le_mod_order(&seed_real);
        let k = PrivateKey::<CurrentNetwork>::try_from(FromBytes::read_le(&*field.to_bytes_le().unwrap()).unwrap()).unwrap();
        println!("from seed = {}", k.to_string());
    }
